...
int main(void)
{
    ...
    double opt_threadpool_sum_a = 0.0, opt_threadpool_sum_f = 0.0, opt_threadpool_a, opt_threadpool_f;
    for (int i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
        ...
        fscanf(fp, "%s %s %lf %lf\n", append, find,&opt_threadpool_a, &opt_threadpool_f);
        opt_threadpool_sum_a += opt_threadpool_a;
        opt_threadpool_sum_f += opt_threadpool_f;
    }
    fclose(fp);
    ...
    fprintf(output, "append() %lf %lf %lf\n", orig_sum_a / 100.0,
            opt_sum_a / 100.0, opt_threadpool_sum_a / 100.0);
    fprintf(output, "findName() %lf %lf %lf", orig_sum_f / 100.0,
            opt_sum_f / 100.0, opt_threadpool_sum_a / 100.0);
    fclose(output);
    return 0;
}
